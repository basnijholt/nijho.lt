...
mlx-whisper = ["mlx-whisper>=0.4.0; sys_platform == 'darwin'"]
...
A subtle but important detail: you don't want to *import* `torch` just to check if it's available.
That takes seconds and loads hundreds of megabytes into memory.
Instead, I ask Python's import machinery if it *could* import it:
...
`find_spec` checks whether a module exists on the import path without executing any of its code.
...
Both are generated from `pyproject.toml` and `uv.lock` by [scripts](https://github.com/basnijholt/agent-cli/blob/c9a67b484331b64733edba8d72d20523699c5a72/.github/scripts/sync_extras.py) that run as pre-commit hooks.
A [separate check script](https://github.com/basnijholt/agent-cli/blob/c9a67b484331b64733edba8d72d20523699c5a72/.github/scripts/check_extras_sync.py) validates that everything is consistent—if you add an extra to `pyproject.toml` but forget to add its metadata, the commit fails with a clear error.
...
The implementation lives primarily in [`agent_cli/core/deps.py`](https://github.com/basnijholt/agent-cli/blob/main/agent_cli/core/deps.py).*
